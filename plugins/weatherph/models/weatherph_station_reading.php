...
//                $result['min'] = date('m',$utcDate);
    ...
    public function getStationReadings($conditions = array()) {
        ...
        $stationId = $conditions['conditions']['id'];
        $startdate = $conditions['conditions']['startdate'];
        $enddate = $conditions['conditions']['enddate'];
        $timeinterval = $conditions['conditions']['timeinterval'];

        $readings = $Reading->find('all', array(
            'conditions' => array(
                'ort1 LIKE' => '%' . $stationId . '%',
                'date(datum) BETWEEN ? AND ?' => array($startdate, $enddate),
            ),
            'order' => 'datum,utc,min ASC',
                ));

        foreach ($readings as $reading) {
            $reading['dir'] = ($reading['dir'] == '')? '-' : round($reading['dir'],0);   
            $reading['ff'] = ($reading['ff']  == '')? '-' : round($reading['ff'],0);
            $reading['tl'] = ($reading['tl'] == '')? '-' : round($reading['tl'],0);
            $reading['tn'] = ($reading['tl'] == '')? '-' : round($reading['tl'],0);;
            $reading['tx'] = ($reading['tl'] == '')? '-' : round($reading['tl'],0);;
            $reading['td'] = ($reading['tl'] == '')? '-' : round($reading['tl'],0);;
            $reading['rr1h'] = ($reading['tl'] == '')? '-' : round($reading['tl'],0);;
            $reading['rain3'] = ($reading['tl'] == '')? '-' : round($reading['tl'],0);;
            $reading['rain6'] = ($reading['tl'] == '')? '-' : round($reading['tl'],0);;
            $reading['g1h'] = ($reading['tl'] == '')? '-' : round($reading['tl'],0);;
            $reading['gl1h'] = ($reading['tl'] == '')? '-' : round($reading['tl'],0);;
            $reading['sy'] = ($reading['tl'] == '')? '-' : round($reading['tl'],0);;
            $reading['sy2'] = ($reading['tl'] == '')? '-' : round($reading['tl'],0);;
