...
foreach($two_week_readings as $keyId=>$station){
    
    $station_name = explode('/', $keyId);
    
    echo '<table>'; 
    echo '<tr><td colspan="17" class="stations_name">'.$station_name[1].' - ' .$station_name[0]. '</td></tr>';
    
    foreach($station as $keyDate=>$readingDates){
        
        echo '<tr><td colspan="17">'.date('F m, Y', strtotime($keyDate)).'</td></td>';
