...
//$enddate = date('Y-m-d H:i:s', strtotime('+1 hour', $enddate));
if (isset($argv[1])) {
    $res = "+1 hour";
    $startdate = date('Y-m-d 12:00:00');
    $enddate = date('Y-m-d 15:00:00');
    $enddate = date('Y-m-d H:i:s', strtotime($res, strtotime($enddate)));
}
$STATIC = array(
...
foreach ($coordinates as $coordinate_key => $coordinate_value) {
    foreach ($properties as $property_key => $property_value) {
        while (strtotime($start) < strtotime($end)) {
            ...
            echo $c->h($property_value, 'yellow') . 'dt=' . $c->h($start, 'blue') . '&' . $c->h($coordinate_value, 'orange') . "\n";

            $counter++;
            exec("wget -O '$location/$start{$coordinate_key}_{$property_key}.png' '$url'");
            $filetotal += filesize("$location/$start{$coordinate_key}_{$property_key}.png");
            $file_mb = round(($filetotal / 1048576), 2);
            echo "\n\n" . $c->h($start, 'blue') . "\n\n";
            $start = date('YmdHis', strtotime($res, strtotime($start)));
            $reg[] = "$start{$coordinate_key}_{$property_key}.png";
        }
        $start = $STATIC['start'];
    }
}
...
echo $ready;
exec('cd Zip');
echo "zip -r "."dl_layers".$name.'.zip '.$ready;
exec('zip -r ' . 'dl_layers' . $name . '.zip ' . $ready);
echo "\n\n";
